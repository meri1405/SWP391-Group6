...
const VaccinationCampaignDetail = ({ campaignId, onBack, onEdit }) => {
  ...
  const [eligibleStudents, setEligibleStudents] = useState({ eligibleStudents: [], ineligibleStudents: [] });
  ...
  const fetchCampaignData = async () => {
    ...
    try {
      ...
      try {
        ...
        if (students.error || (students.message && students.message.includes('chưa được phê duyệt'))) {
  ...
  const handleVaccinationResult = async (recordData) => {
    try {
      ...
      await vaccinationCampaignApi.createVaccinationRecord(selectedForm.id, recordData);
        ...
        vaccinations && vaccinations.length > 0 ? (
          <ul style={{ paddingLeft: '20px', margin: 0 }}>
            {vaccinations.map((v, index) => (
              <li key={index}>
                {v.vaccineName} (liều {v.doseNumber}) - {dayjs(v.dateOfVaccination).format('DD/MM/YYYY')}
              </li>
            ))}
          </ul>
        ) : (
  ...
  const vaccinationFormsColumns = [
    ...
    },
    {
      ...
      dataIndex: 'sentDate',
      key: 'sentDate',
      render: date => date ? dayjs(date).format('DD/MM/YYYY HH:mm') : 'Chưa gửi',
    },
    {
      title: 'Hành động',
      key: 'action',
      render: (_, record) => (
        <Space size="small">
          {record.confirmationStatus === 'CONFIRMED' && !records.find(r => r.vaccinationFormId === record.id) ? (
            <Button 
              type="primary" 
              size="small" 
  ...
  const vaccinationRecordsColumns = [
    ...
    },
    {
      ...
      key: 'notes',
      render: (notes, record) => {
        if (record.preVaccinationStatus === 'ABNORMAL' || record.preVaccinationStatus === 'POSTPONED') {
          return record.preVaccinationNotes || 'Không có';
        }
        return notes || 'Không có';
      }
    },
    {
      title: 'Hành động',
    ...
    return [
      {
        ...
        children: (
          <Table 
            ...
            dataSource={eligibleStudents.eligibleStudents?.map((student, i) => ({ ...student, key: `eligible-${i}` })) || []}
      ...
      },
      {
        ...
        children: (
          <Table 
            ...
            dataSource={eligibleStudents.ineligibleStudents?.map((student, i) => ({ ...student, key: `ineligible-${i}` })) || []}
            pagination={{ pageSize: 10 }}
            loading={loading}
          />
        )
      },
      {
        key: '3',
        label: (
          <Badge count={forms.length} style={{ backgroundColor: forms.length ? '#1890ff' : '#d9d9d9' }}>
      ...
      },
      {
        key: '4',
        label: (
          <Badge count={records.length} style={{ backgroundColor: records.length ? '#52c41a' : '#d9d9d9' }}>
    ...
    return (
      <Card>
        <div style={{ textAlign: 'center', padding: '20px' }}>
          <ExclamationCircleOutlined style={{ fontSize: '48px', color: '#f5222d' }} />
          <Title level={4} style={{ marginTop: '16px' }}>Không tìm thấy thông tin chiến dịch</Title>
          <Button type="primary" onClick={onBack} icon={<ArrowLeftOutlined />} style={{ marginTop: '16px' }}>
            Quay lại
          </Button>
        </div>
      </Card>
  ...
  const confirmedForms = forms.filter(form => form.confirmationStatus === 'CONFIRMED').length;
  const rejectedForms = forms.filter(form => form.confirmationStatus === 'REJECTED').length;
  const pendingForms = forms.filter(form => form.confirmationStatus === 'PENDING').length;
  const completedRecords = records.filter(record => record.preVaccinationStatus === 'NORMAL').length;
  const postponedRecords = records.filter(record => 
    record.preVaccinationStatus === 'ABNORMAL' || record.preVaccinationStatus === 'POSTPONED'
  ).length;
    ...
    <div className="vaccination-campaign-detail">
      <Card>
        ...
        <Descriptions title="Thông tin chiến dịch" bordered column={2}>
          ...
          <Descriptions.Item label="Loại vắc xin">{campaign.vaccineName}</Descriptions.Item>
          <Descriptions.Item label="Nhãn hiệu">{campaign.vaccineBrand || 'N/A'}</Descriptions.Item>
          <Descriptions.Item label="Liều số">{campaign.doseNumber || 'N/A'}</Descriptions.Item>
          ...
          <Descriptions.Item label="Trạng thái">{getStatusTag(campaign.status)}</Descriptions.Item>
          <Descriptions.Item label="Số vắc xin dự kiến">{campaign.estimatedVaccineCount}</Descriptions.Item>
      ...
      >
        <p>Bạn có chắc chắn muốn hoàn thành chiến dịch tiêm chủng này? Hành động này không thể hoàn tác.</p>
        <p>Tổng số học sinh đã tiêm: <strong>{completedRecords}</strong></p>
        <p>Tổng số học sinh hoãn tiêm: <strong>{postponedRecords}</strong></p>
        <p>Tổng số mẫu đơn phụ huynh chưa xác nhận: <strong>{pendingForms}</strong></p>
      </Modal>
    </div>
  );
};
