...
const VaccinationCampaignForm = ({ campaign = null, onCancel, onSuccess }) => {
  ...
  useEffect(() => {
    ...
    if (isEditing) {
      ...
      form.setFieldsValue({
        ...campaign,
        scheduledDate: campaign.scheduledDate ? dayjs(campaign.scheduledDate) : null,
      });
    }
  ...
  const handleSubmit = async (values) => {
    ...
    try {
      const campaignData = {
        ...values,
        scheduledDate: values.scheduledDate ? values.scheduledDate.format('YYYY-MM-DDTHH:mm:ss') : null,
      };

      let result;
      if (isEditing) {
        result = await vaccinationCampaignApi.updateCampaign(campaign.id, campaignData);
      ...
      message.error(`Không thể ${isEditing ? 'cập nhật' : 'tạo'} chiến dịch tiêm chủng`);
  ...
  const onRuleChange = (ruleId) => {
    ...
    if (selectedRule) {
      form.setFieldsValue({
        ...
        additionalInfo: `Vắc xin ${selectedRule.name}, liều ${selectedRule.doesNumber}.\n${selectedRule.description || ''}`
      });
    }
  };
  ...
  return (
    <Card 
      title={
        <Title level={4}>
          {isEditing ? 'Chỉnh sửa chiến dịch tiêm chủng' : 'Tạo chiến dịch tiêm chủng mới'}
        </Title>
      }
    >
      <Form
      ...
      >
        <Row gutter={16}>
          ...
          <Col span={12}>
            <Form.Item
              ...
              rules={[{ required: true, message: 'Vui lòng chọn quy tắc tiêm chủng' }]}
            >
              <Select 
        ...
        <Row gutter={16}>
          ...
          <Col span={12}>
            <Form.Item
              ...
              rules={[{ required: true, message: 'Vui lòng nhập số lượng vắc xin' }]}
            >
              <InputNumber 
                min={1} 
                style={{ width: '100%' }}
        ...
        >
          <TextArea rows={3} placeholder="Nhập mô tả chi tiết về chiến dịch tiêm chủng" />
        </Form.Item>
        ...
        >
          <TextArea rows={4} placeholder="Hướng dẫn chăm sóc trước và sau tiêm" />
        </Form.Item>
