...
const VaccinationCampaignList = ({ onCreateNew, onViewDetails }) => {
    ...
    try {
      ...
      if (dateRange && dateRange.length === 2) {
        ...
        filteredData = filteredData.filter((campaign) => {
          ...
          return (
            campaignDate.isAfter(startDate) && campaignDate.isBefore(endDate)
          );
        });
      }
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      message.error(
        "Không thể xóa chiến dịch. Chỉ có thể xóa chiến dịch ở trạng thái CHƯA DUYỆT."
      );
  ...
  const handleEdit = (campaign) => {
    if (campaign.status !== "PENDING") {
      message.warning(
        "Chỉ có thể chỉnh sửa chiến dịch ở trạng thái CHƯA DUYỆT"
      );
      return;
    }
  ...
  return (
    <>
      <Card
        title="Danh sách chiến dịch tiêm chủng"
        extra={
          <Button
            type="primary"
            icon={<PlusOutlined />}
