...
const RecordResultsTab = ({ campaign, campaignId, onRefreshData }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      console.log(
        "Making API call to getConfirmedStudents with campaignId:",
        campaignId
      );
      try {
        ...
        console.log(
          "API call successful - confirmed students response:",
          students
        );
        ...
        console.log(
          "Fetching existing health check results for campaign:",
          campaignId
        );
        ...
        if (results && Array.isArray(results)) {
          results.forEach((result) => {
            if (result.studentID) {
              ...
              console.log(
                `Student ${result.studentID} has existing results:`,
                result
              );
            }
          });
  ...
  const handleViewResult = (student) => {
    ...
    console.log(
      "Loading existing results for student:",
      student.studentID,
      studentResults
    );
    ...
    if (studentResults) {
      ...
      const loadedData = {
        weight:
          studentResults.weight || studentResults.overallResults?.weight || "",
        height:
          studentResults.height || studentResults.overallResults?.height || "",
      };
      ...
      if (campaign?.categories) {
        campaign.categories.forEach((category) => {
          console.log(
            `Processing category ${category} for student ${student.studentID}`
          );
          ...
          if (studentResults.results && studentResults.results[category]) {
            ...
            console.log(
              `Found category data in results.${category}:`,
              categoryData
            );
          }
          // Fallback: try direct category key
            ...
            console.log(
              `Found category data with direct key ${category}:`,
              categoryData
            );
          }
          ...
          if (categoryData) {
            ...
            // Map common fields
            if (categoryData.doctorName)
              mappedData.doctorName = categoryData.doctorName;
            if (categoryData.isAbnormal !== undefined)
              mappedData.isAbnormal = categoryData.isAbnormal;
            if (categoryData.recommendations)
              mappedData.recommendations = categoryData.recommendations;
            if (categoryData.dateOfExamination)
              mappedData.dateOfExamination = categoryData.dateOfExamination;

            // Map category-specific fields based on category type
            if (category === "VISION") {
              if (categoryData.visionLeft !== undefined)
                mappedData.visionLeft = categoryData.visionLeft;
              if (categoryData.visionRight !== undefined)
                mappedData.visionRight = categoryData.visionRight;
              if (categoryData.visionLeftWithGlass !== undefined)
                mappedData.visionLeftWithGlass =
                  categoryData.visionLeftWithGlass;
              if (categoryData.visionRightWithGlass !== undefined)
                mappedData.visionRightWithGlass =
                  categoryData.visionRightWithGlass;
              if (categoryData.visionDescription)
                mappedData.visionDescription = categoryData.visionDescription;
              if (categoryData.eyeMovement)
                mappedData.eyeMovement = categoryData.eyeMovement;
              if (categoryData.eyePressure !== undefined)
                mappedData.eyePressure = categoryData.eyePressure;
              if (categoryData.needsGlasses !== undefined)
                mappedData.needsGlasses = categoryData.needsGlasses;
            } else if (category === "HEARING") {
              if (categoryData.leftEar !== undefined)
                mappedData.leftEar = categoryData.leftEar;
              if (categoryData.rightEar !== undefined)
                mappedData.rightEar = categoryData.rightEar;
              if (categoryData.description)
                mappedData.description = categoryData.description;
              if (categoryData.hearingAcuity)
                mappedData.hearingAcuity = categoryData.hearingAcuity;
              if (categoryData.tympanometry)
                mappedData.tympanometry = categoryData.tympanometry;
              if (categoryData.earWaxPresent !== undefined)
                mappedData.earWaxPresent = categoryData.earWaxPresent;
              if (categoryData.earInfection !== undefined)
                mappedData.earInfection = categoryData.earInfection;
            } else if (category === "ORAL") {
              if (categoryData.teethCondition)
                mappedData.teethCondition = categoryData.teethCondition;
              if (categoryData.gumsCondition)
                mappedData.gumsCondition = categoryData.gumsCondition;
              if (categoryData.tongueCondition)
                mappedData.tongueCondition = categoryData.tongueCondition;
              if (categoryData.description)
                mappedData.description = categoryData.description;
              if (categoryData.oralHygiene)
                mappedData.oralHygiene = categoryData.oralHygiene;
              if (categoryData.cavitiesCount !== undefined)
                mappedData.cavitiesCount = categoryData.cavitiesCount;
              if (categoryData.plaquePresent !== undefined)
                mappedData.plaquePresent = categoryData.plaquePresent;
              if (categoryData.gingivitis !== undefined)
                mappedData.gingivitis = categoryData.gingivitis;
              if (categoryData.mouthUlcers !== undefined)
                mappedData.mouthUlcers = categoryData.mouthUlcers;
            } else if (category === "SKIN") {
              if (categoryData.skinColor)
                mappedData.skinColor = categoryData.skinColor;
              if (categoryData.rashes !== undefined)
                mappedData.rashes = categoryData.rashes;
              if (categoryData.lesions !== undefined)
                mappedData.lesions = categoryData.lesions;
              if (categoryData.dryness !== undefined)
                mappedData.dryness = categoryData.dryness;
              if (categoryData.eczema !== undefined)
                mappedData.eczema = categoryData.eczema;
              if (categoryData.psoriasis !== undefined)
                mappedData.psoriasis = categoryData.psoriasis;
              if (categoryData.skinInfection !== undefined)
                mappedData.skinInfection = categoryData.skinInfection;
              if (categoryData.allergies !== undefined)
                mappedData.allergies = categoryData.allergies;
              if (categoryData.acne !== undefined)
                mappedData.acne = categoryData.acne;
              if (categoryData.scars !== undefined)
                mappedData.scars = categoryData.scars;
              if (categoryData.birthmarks !== undefined)
                mappedData.birthmarks = categoryData.birthmarks;
              if (categoryData.skinTone)
                mappedData.skinTone = categoryData.skinTone;
              if (categoryData.description)
                mappedData.description = categoryData.description;
              if (categoryData.treatment)
                mappedData.treatment = categoryData.treatment;
              if (categoryData.followUpDate)
                mappedData.followUpDate = categoryData.followUpDate;
            } else if (category === "RESPIRATORY") {
              if (categoryData.breathingRate !== undefined)
                mappedData.breathingRate = categoryData.breathingRate;
              if (categoryData.breathingSound)
                mappedData.breathingSound = categoryData.breathingSound;
              if (categoryData.wheezing !== undefined)
                mappedData.wheezing = categoryData.wheezing;
              if (categoryData.cough !== undefined)
                mappedData.cough = categoryData.cough;
              if (categoryData.breathingDifficulty !== undefined)
                mappedData.breathingDifficulty =
                  categoryData.breathingDifficulty;
              if (categoryData.oxygenSaturation !== undefined)
                mappedData.oxygenSaturation = categoryData.oxygenSaturation;
              if (categoryData.chestExpansion)
                mappedData.chestExpansion = categoryData.chestExpansion;
              if (categoryData.lungSounds)
                mappedData.lungSounds = categoryData.lungSounds;
              if (categoryData.asthmaHistory !== undefined)
                mappedData.asthmaHistory = categoryData.asthmaHistory;
              if (categoryData.allergicRhinitis !== undefined)
                mappedData.allergicRhinitis = categoryData.allergicRhinitis;
              if (categoryData.treatment)
                mappedData.treatment = categoryData.treatment;
              if (categoryData.description)
                mappedData.description = categoryData.description;
            ...
            console.log(
              `No existing data found for category ${category}, using initial data`
            );
    ...
    },
    {
      ...
      render: (_, record) => {
        ...
        return hasResults ? (
          ...
          <Tag color="processing">Chờ khám</Tag>
        );
      },
      align: "center",
    },
            ...
            <Space direction="vertical" style={{ width: "100%" }}>
              <div>
                ...
                <InputNumber
                  ...
                  onChange={(value) =>
                    handleInputChange("VISION", "visionLeft", value || 0)
                  }
              ...
              <div>
                ...
                <InputNumber
                  ...
                  onChange={(value) =>
                    handleInputChange("VISION", "visionRight", value || 0)
                  }
                  placeholder="Ví dụ: 10/10"
                  style={{ width: "100%", marginTop: 4 }}
                />
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      try {
        const updatedResults = await healthCheckApi.getCampaignResults(
          campaignId
        );
        const resultsMap = {};
  ...
  // Debug logging
  console.log(
    "RecordResultsTab - confirmedStudents from state:",
    confirmedStudents
  );
  console.log(
    "RecordResultsTab - confirmedStudents length:",
                ...
                <Row justify="space-between" align="middle">
                  ...
                  <Col>
                    <div style={{ textAlign: "right" }}>
                      ...
                      >
                        {modalMode === "record" ? "Ngày khám" : "Trạng thái"}
                      </Text>
                      <div
                        style={{
                          color: "white",
              ...
              >
                ...
                {campaign?.categories?.map((category) => (
                  <div key={category}>
                    {category === "VISION" &&
                      formData.VISION &&
                      renderVisionForm(formData.VISION)}
                    {category === "HEARING" &&
                      formData.HEARING &&
                      renderHearingForm(formData.HEARING)}
                    {category === "ORAL" &&
                      formData.ORAL &&
                      renderOralForm(formData.ORAL)}
                    {category === "SKIN" &&
                      formData.SKIN &&
                      renderSkinForm(formData.SKIN)}
                    {category === "RESPIRATORY" &&
                      formData.RESPIRATORY &&
                      renderRespiratoryForm(formData.RESPIRATORY)}
                  </div>
                ))}
