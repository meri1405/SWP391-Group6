...
const HealthProfileDetailModal = ({ visible, onClose, healthProfile }) => {
  ...
  console.log(
    "HealthProfile keys:",
    healthProfile ? Object.keys(healthProfile) : "no healthProfile"
  );
  ...
  console.log(
    ...
    healthProfile?.treatmentHistory?.length
  );
  ...
  const studentName =
    student?.firstName && student?.lastName
      ...
      : student?.fullName ||
        ...
        healthProfile.student?.fullName ||
        "N/A";
    ...
    >
      ...
      >
                  ...
                  <Card size="small" style={{ width: "100%" }}>
                    ...
                    <Row style={{ marginTop: 8 }}>
                      <Col span={24}>
                        ...
                        <Text>
                          {disease.placeOfTreatment || "Không có thông tin"}
                        </Text>
                      </Col>
                    </Row>
                    <Row style={{ marginTop: 8 }}>
                    ...
                    <Row style={{ marginTop: 8 }}>
                      <Col span={24}>
                        ...
                        {disease.status === "UNDER_TREATMENT" && (
                          <Tag color="blue">Đang điều trị</Tag>
                        )}
                        {disease.status === "RECOVERED" && (
                          <Tag color="green">Đã khỏi</Tag>
                        )}
                        {disease.status === "STABLE" && (
                          <Tag color="cyan">Ổn định</Tag>
                        )}
                        {disease.status === "WORSENED" && (
                          <Tag color="red">Xấu đi</Tag>
                        )}
                        {disease.status === "RELAPSED" && (
                          <Tag color="orange">Tái phát</Tag>
                        )}
                        {disease.status === "NEWLY_DIAGNOSED" && (
                          <Tag color="purple">Mới chẩn đoán</Tag>
                        )}
                        {disease.status === "UNDER_OBSERVATION" && (
                          <Tag color="geekblue">Đang theo dõi</Tag>
                        )}
                        {disease.status === "UNTREATED" && (
                          <Tag color="magenta">Chưa điều trị</Tag>
                        )}
                        ...
                        {![
                          ...
                          "UNKNOWN",
                          "UNTREATED",
                        ].includes(disease.status) && (
                          <Tag color="default">
                            {disease.status || "Không xác định"}
                          </Tag>
                        )}
                      </Col>
                    </Row>
        ...
        <TabPane
          ...
          key="infectious"
        >
          {healthProfile.infectiousDiseases &&
          healthProfile.infectiousDiseases.length > 0 ? (
            <List
                    ...
                    <Row style={{ marginTop: 8 }}>
                      <Col span={24}>
                        ...
                        {disease.status === "ISOLATED" && (
                          <Tag color="volcano">Đang cách ly</Tag>
                        )}
                        {disease.status === "UNDER_TREATMENT" && (
                          <Tag color="blue">Đang điều trị</Tag>
                        )}
        ...
        <TabPane
          tab={
            <span>
              <MedicineBoxOutlined />
              Lịch sử điều trị (
              {(healthProfile.treatments || healthProfile.treatmentHistory)
                ?.length || 0}
              )
            </span>
          }
        ...
        >
          {(healthProfile.treatments || healthProfile.treatmentHistory) &&
          (healthProfile.treatments || healthProfile.treatmentHistory).length >
            0 ? (
            <List
              dataSource={
                healthProfile.treatments || healthProfile.treatmentHistory
                  ...
                  <Card size="small" style={{ width: "100%" }}>
                    ...
                    <Row style={{ marginTop: 8 }}>
                      <Col span={24}>
                        <Text strong>Nơi điều trị: </Text>
                        <Text>
                          {treatment.placeOfTreatment || "Không có thông tin"}
                        </Text>
                      </Col>
                    </Row>
                    <Row style={{ marginTop: 8 }}>
                    ...
                    {treatment.status && (
                      <Row style={{ marginTop: 8 }}>
                        <Col span={24}>
                          <Text strong>Tình trạng: </Text>
                          {treatment.status === "UNDER_TREATMENT" && (
                            <Tag color="blue">Đang điều trị</Tag>
                          )}
                          ...
                          {treatment.status === "WORSENED" && (
                            <Tag color="red">Xấu đi</Tag>
                          )}
                          {treatment.status === "RELAPSED" && (
                            <Tag color="orange">Tái phát</Tag>
                          )}
                          {treatment.status === "NEWLY_DIAGNOSED" && (
                            <Tag color="purple">Mới chẩn đoán</Tag>
                          )}
                          {treatment.status === "UNDER_OBSERVATION" && (
                            <Tag color="geekblue">Đang theo dõi</Tag>
                          )}
                          ...
                          {![
                            ...
                            <Tag color="default">
                              {treatment.status || "Không xác định"}
                            </Tag>
                          )}
                        </Col>
                      </Row>
                  ...
                  <Card size="small" style={{ width: "100%" }}>
                    ...
                    <Row style={{ marginTop: 8 }}>
                      <Col span={24}>
                        <Text strong>Mô tả: </Text>
                        <Text>
                          {vision.visionDescription || "Không có mô tả"}
                        </Text>
                      </Col>
                    </Row>
                  </Card>
        ...
        <TabPane
          tab={
            <span>
              <SafetyOutlined />
              Lịch sử tiêm chủng (
              {healthProfile.vaccinationHistory?.length || 0})
            </span>
          }
          key="vaccination"
        >
                  ...
                  <Card size="small" style={{ width: "100%" }}>
                    ...
                    <Row style={{ marginTop: 8 }} gutter={16}>
                      <Col span={12}>
                        ...
                        <Text>
                          {vaccination.manufacturer || "Không có thông tin"}
                        </Text>
                      </Col>
                      <Col span={12}>
                    ...
                    <Row style={{ marginTop: 8 }} gutter={16}>
                      ...
                      <Col span={12}>
                        ...
                        <Text>
                          {vaccination.administeredBy || "Không có thông tin"}
                        </Text>
                      </Col>
                    </Row>{" "}
                    <Row style={{ marginTop: 8 }}>
                      <Col span={12}>
                        <Text strong>Trạng thái: </Text>
                        <Tag
                          color={vaccination.status === true ? "green" : "red"}
                        >
