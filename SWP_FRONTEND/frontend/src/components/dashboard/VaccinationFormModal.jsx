...
const VaccinationFormModal = ({ 
  ...
  const loadVaccinationForm = useCallback(async () => {
    ...
    try {
      setLoading(true);
      setError(null);
      const response = await parentApi.getVaccinationFormById(vaccinationFormId, token);
      if (response.success) {
  ...
  const handleConfirmSubmit = async () => {
    ...
    try {
      ...
      if (confirmAction === 'confirm') {
        response = await parentApi.confirmVaccinationForm(vaccinationFormId, notes, token);
      } else {
        response = await parentApi.declineVaccinationForm(vaccinationFormId, notes, token);
      }
    ...
    <div className="vaccination-form-modal-overlay">
      <div className="vaccination-form-modal">
        ...
        <div className="vaccination-form-modal-body">
          ...
          {form && (
            <div className="form-details">              <div className="form-header">
                <div className="student-info">
                  <h3>{form.studentFullName}</h3>
                  <div className="student-details">
                    {form.studentDateOfBirth && (
                      <p><i className="fas fa-birthday-cake"></i> Ngày sinh: {formatDate(form.studentDateOfBirth)}</p>
                    )}
                    {form.studentGender && (
                      <p><i className="fas fa-user"></i> Giới tính: {formatGender(form.studentGender)}</p>
                    )}
                    {form.studentClassName && (
                      <p><i className="fas fa-graduation-cap"></i> Lớp: {form.studentClassName}</p>
                    )}
                  </div>
                </div>
                <div className="status-badge" style={{ backgroundColor: getStatusColor(form.confirmationStatus) }}>
            ...
            <div className="confirm-dialog">
              ...
              <div className="confirm-dialog-body">
                ...
                <div className="notes-section">
                  <label>Ghi chú {confirmAction === 'decline' ? '(bắt buộc)' : '(tùy chọn)'}:</label>
                  <textarea
                    value={notes}
                    onChange={(e) => setNotes(e.target.value)}
                    placeholder={confirmAction === 'confirm' 
                      ? "Ghi chú thêm (nếu có)..."
              ...
              <div className="confirm-dialog-actions">
                ...
                <button 
                  ...
                  disabled={submitting || (confirmAction === 'decline' && !notes.trim())}
                >
                  {submitting ? (
                    <>
                      <i className="fas fa-spinner fa-spin"></i>
