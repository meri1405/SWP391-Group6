  ...
  const [publishedPosts, setPublishedPosts] = useState([
    {
      ...
      content:
        "Bài viết hướng dẫn các biện pháp phòng chống dịch sốt xuất huyết...",
    ...
    Modal.confirm({
      ...
      onOk() {
        if (activeTab === "published") {
          setPublishedPosts(
            publishedPosts.filter((item) => item.id !== record.id)
          );
        } else {
    ...
    Modal.info({
      ...
      content: (
        <div>
          ...
          />
          <p>
            <strong>Tác giả:</strong> {record.author}
          </p>
          <p>
            <strong>Danh mục:</strong> {getCategoryTag(record.category)}
          </p>
          <p>
            <strong>Ngày {activeTab === "published" ? "đăng" : "tạo"}:</strong>{" "}
            {dayjs(
              activeTab === "published"
                ? record.publishDate
                : record.createdDate
          ...
          <div style={{ marginTop: 16 }}>{record.content}</div>
        </div>
      ),
    });
  };
