...
const MedicalSupplyInventory = () => {
  ...
  useEffect(() => {
    ...
    if (categoryFilter && !validValues.includes(categoryFilter)) {
      console.warn(`Invalid category filter value: ${categoryFilter}. Resetting to 'all'`);
  ...
  useEffect(() => {
    const loadData = async () => {
      console.log('Effect triggered: fetch data with', { tab: activeTab, category: categoryFilter, search: searchText });
  ...
  const fetchSupplies = async (
    ...
    try {
      ...
      console.log(`Fetching data for tab: ${tab}, category: ${category}, search: ${search}`);
      ...
      if (category !== 'all') {
        ...
        data = data.filter(item => {
          ...
          if (category === 'bandage' && ['sơ cứu', 'so cuu', 'first aid'].includes(itemCategoryLower)) return true;
          if (category === 'medicine' && ['thuốc', 'thuoc', 'drug'].includes(itemCategoryLower)) return true;
          if (category === 'equipment' && ['thiết bị', 'thiet bi', 'device'].includes(itemCategoryLower)) return true;
          if (category === 'consumable' && ['vật tư tiêu hao', 'vat tu tieu hao'].includes(itemCategoryLower)) return true;
  ...
  const calculateSupplyStatus = (supply) => {
    ...
    const isExpiringSoon = !isExpired && expirationDate.isBefore(threeMonthsLater, 'day');
    ...
    return {
      ...
      status: isExpired ? 'expired' : 
              ...
              isExpiringSoon ? 'expiring-soon' : 'normal'
    };
  };
  ...
  const getSupplyStatusTag = (supply) => {
    ...
    if (status.isExpired) {
      ...
      return <Tag color="gold" icon={<WarningOutlined />}>Sắp hết hạn</Tag>;
    } else {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (date, record) => {
        ...
        return (
          <Typography.Text
            type={status.isExpired ? 'danger' : status.isExpiringSoon ? 'warning' : undefined}
            style={{ fontWeight: status.isExpired || status.isExpiringSoon ? 'bold' : 'normal' }}
          >
            {formattedDate}
            {status.isExpired && (
            ...
            {status.isExpiringSoon && !status.isExpired && (
              <Tooltip title={`Sắp hết hạn (còn ${dayjs(date).diff(dayjs(), 'day')} ngày)`}>
    ...
    Modal.info({
      ...
      content: (
        <div style={{ marginTop: '20px' }}>
          <p><strong>Loại:</strong> {getCategoryTag(supply.category)}</p>
          <p><strong>Số lượng:</strong> {supply.quantity} {supply.unit}</p>
          <p><strong>Mức tồn kho tối thiểu:</strong> {supply.minStockLevel} {supply.unit}</p>
          <p>
            <strong>Hạn sử dụng:</strong> {dayjs(supply.expirationDate).format('DD/MM/YYYY')} 
            {calculateSupplyStatus(supply).isExpired && (
              <Tag color="red" style={{ marginLeft: 8 }}>Đã hết hạn</Tag>
            )}
            {calculateSupplyStatus(supply).isExpiringSoon && !calculateSupplyStatus(supply).isExpired && (
              <Tag color="orange" style={{ marginLeft: 8 }}>
                Sắp hết hạn (còn {dayjs(supply.expirationDate).diff(dayjs(), 'day')} ngày)
              </Tag>
            )}
          </p>
          <p><strong>Nhà cung cấp:</strong> {supply.supplier}</p>
          <p><strong>Vị trí:</strong> {supply.location}</p>
          {supply.description && (
            <p><strong>Mô tả:</strong> {supply.description}</p>
          )}
          <p><strong>Trạng thái:</strong> {getSupplyStatusTag(supply)}</p>
          <p><strong>Ngày cập nhật:</strong> {dayjs(supply.updatedAt).format('DD/MM/YYYY HH:mm')}</p>
        </div>
      ),
      okText: 'Đóng',
    });
  ...
  const calculateTabCounts = () => {
    if (!supplies || supplies.length === 0) return { all: 0, lowStock: 0, expiringSoon: 0, expired: 0 };
    
    const counts = { all: supplies.length, lowStock: 0, expiringSoon: 0, expired: 0 };
          ...
          <div>
            ...
            {categoryFilter !== 'all' && (
              <div style={{ marginTop: -8 }}>
                <Tag color="blue">
                  Đang lọc theo loại: {categories.find(c => c.value === categoryFilter)?.label || categoryFilter}
                </Tag>
              </div>
            )}
          </div>
            ...
            {categories.map(category => {
              ...
              const count = category.value === 'all' 
                ...
                : supplies.filter(item => 
                    item.category && item.category.toLowerCase() === category.value.toLowerCase()
                  ).length;
              ...
              return (
                ...
                >
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    ...
                    <span style={{ 
                      color: categoryFilter === category.value ? '#1890ff' : '#999',
                      fontWeight: categoryFilter === category.value ? 'bold' : 'normal'
                    }}>
                      {count}
                    </span>
                  </div>
        ...
        <Table
          ...
          locale={{ 
            emptyText: categoryFilter !== 'all' 
              ? `Không tìm thấy vật tư y tế loại "${categories.find(c => c.value === categoryFilter)?.label || categoryFilter}"` 
