...
const NotificationsSection = () => {
  ...
  const [sentNotifications, setSentNotifications] = useState([
    {
      ...
      type: "health_check",
      content:
        "Nhà trường sẽ tổ chức khám sức khỏe định kỳ cho học sinh vào ngày 15/02/2024",
  ...
  const getRecipientsTag = (recipients) => {
    ...
    return (
      <Tag color={config[recipients].color}>{config[recipients].text}</Tag>
    );
  };
      ...
      onOk() {
        if (activeTab === "sent") {
          setSentNotifications(
            sentNotifications.filter((item) => item.id !== record.id)
          );
        } else {
          setDraftNotifications(
            draftNotifications.filter((item) => item.id !== record.id)
          );
        }
