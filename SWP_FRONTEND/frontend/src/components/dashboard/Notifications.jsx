...
const Notifications = () => {
  ...
  const getNotificationType = (notification) => {
    if (notification.medicationRequest) {
      ...
      notification.title?.toLowerCase().includes("medical event") ||
      notification.message?.toLowerCase().includes("medical event") ||
      notification.title?.toLowerCase().includes("sự kiện y tế") ||
      notification.message?.toLowerCase().includes("sự kiện y tế")
    ) {
      ...
      notification.message?.toLowerCase().includes("tiêm chủng")
    ) {
      return "vaccination";
    } else if (
      notification.title?.toLowerCase().includes("emergency") ||
      notification.message?.toLowerCase().includes("emergency") ||
      notification.title?.toLowerCase().includes("khẩn cấp") ||
      notification.message?.toLowerCase().includes("khẩn cấp")
    ) {
      return "emergency";
    }
    return "general";
    ...
    <div className="notifications">
      ...
      {showVaccinationModal && (
        <VaccinationFormModal
          ...
          onUpdate={handleVaccinationFormUpdated}
        />
      )}
    </div>
  );
