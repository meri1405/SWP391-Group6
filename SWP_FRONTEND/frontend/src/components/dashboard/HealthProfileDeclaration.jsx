...
const HealthProfileDeclaration = () => {
    ...
    try {
      ...
      const healthProfileData = {
        ...
        chronicDiseases: chronicDiseases.map(disease => {
          ...
          return hearingDataItem;        }) || [],        vaccinationHistory: vaccinationHistory.map(vaccination => {
  ...
  // Helper function to check if health profile editing is allowed
  const isEditingAllowed = () => {
    return healthProfiles.length === 0 || healthProfiles.some(profile => profile.status === 'PENDING');
  };

  // Edit handler functions for each health component
  const handleEditAllergy = (allergy) => {
    if (!isEditingAllowed()) {
      message.warning('Chỉ có thể chỉnh sửa khi hồ sơ có trạng thái "Đang chờ duyệt"');
      return;
    }
  ...
  return (
    <div style={{ padding: '24px', maxWidth: '1200px', margin: '0 auto' }}>
      <Card>
        ...
        )}        {selectedStudent && (healthProfiles.length === 0 || selectedProfile) && (
              ...
              <Divider />
                ...
                <Space>
                  ...
                  <Button
                    ...
                    htmlType="submit"
                    loading={loading}
                    disabled={!selectedStudent || (selectedProfile && selectedProfile.status !== 'PENDING')}
                  >
                    {selectedProfile ? 'Cập nhật hồ sơ sức khỏe' : 'Tạo hồ sơ sức khỏe'}
                  </Button>
                </Space>
                {selectedProfile && selectedProfile.status !== 'PENDING' && (
                  <div style={{ marginTop: '8px', color: '#ff4d4f', fontSize: '14px' }}>
                    Chỉ có thể chỉnh sửa hồ sơ có trạng thái "Đang chờ duyệt"
                  </div>
                )}
              </div>
            </Form>
      ...
      </Card>      {/* Allergy Modal */}      <AllergyModal
      ...
      {/* Chronic Disease Modal */}      <ChronicDiseaseModal
      ...
      {/* Treatment Modal */}      <TreatmentModal
